...
export default class MainScene extends ComponentExtends {
    ...
    loadStoreList() {
        ...
        let nextCall = AsyncQueue.excuteTimes(4, () => {
        ...
        //4
        module.vm.goodsInfo ={multiTotal:1,multiTotalE:0};
        //5
        main.module.vm.propList = null;
        //6
        ...
        //7
        let noviceData = cc.sys.localStorage.getItem("novice");
        if (noviceData) {
            module.vm.noviceProgress = JSON.parse(noviceData) ;
        } else {
            module.vm.noviceProgress = new NoviceProgress();
            cc.sys.localStorage.setItem("novice", JSON.stringify(module.vm.noviceProgress));
        }

        //8
        let mainProgressData = cc.sys.localStorage.getItem("mainProgress");
        if (mainProgressData) {
            module.vm.mainProgress = JSON.parse(mainProgressData);
        } else {
            module.vm.mainProgress = new MainProgress();
            cc.sys.localStorage.setItem("mainProgress", JSON.stringify(module.vm.mainProgress) );
        }

        //9
        let sellNum = cc.sys.localStorage.getItem("sellNum");
        if (sellNum) {
            module.vm.sellNum = sellNum;
        } else {
            module.vm.sellNum = 0;
            cc.sys.localStorage.setItem("sellNum", module.vm.sellNum);
        }
        ...
        //12
        let getFameTime = cc.sys.localStorage.getItem("getFameTime");
        if (getFameTime) {
            module.gamedata.getFameTime = JSON.parse(getFameTime) ;
        } else {
            module.gamedata.getFameTime = main.module.calcUiShow.getSeverCurrentTime() + 3 * 60 * 60 * 1000;
            cc.sys.localStorage.setItem("getFameTime", module.gamedata.getFameTime);
        }


        let fameNpcNum = cc.sys.localStorage.getItem("fameNpcNum");
        if (fameNpcNum) {
            module.gamedata.fameNpcNum = JSON.parse(fameNpcNum)  ;
        } else {
            module.gamedata.fameNpcNum = 0;
            cc.sys.localStorage.setItem("fameNpcNum", module.gamedata.fameNpcNum);
        }


        let isPass = cc.sys.localStorage.getItem("isPass");
        if (isPass) {
            module.vm.isPass = isPass;
        } else {
            module.vm.isPass = 0;
            cc.sys.localStorage.setItem("isPass", module.vm.isPass);
        }
        nextCall();
